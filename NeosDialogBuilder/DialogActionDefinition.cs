...
namespace NeosDialogBuilder
{
    ...
    public class DialogActionDefinition<T> : IDialogEntryDefinition<T> where T : IDialogState
    {
        ...
        public IDialogElement
            Create(UIBuilder uiBuilder, T state, Func<(IDictionary<object, string>, IDictionary<object, string>)> onChange, bool inUserspace = false)
        {
            uiBuilder.PushStyle();
            ...
            button.LocalPressed += (IButton b, ButtonEventData bed) =>
            {
                ...
                if (!element.IsValidating || element.IsValid(state.UpdateAndValidate()))
                {
                    action(state);
                }
            };
